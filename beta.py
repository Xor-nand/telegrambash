...
token = "343244087:AAEGzMyAxH9oZL7ske0RRYx-OCqt2dhd1-M"
...
GOD = 185198389
sudoerfile = open("sudoersbeta.txt", "r")
chatfile = open("chatlist.txt", "r")
...
### TODO update.message.from_user.username
...
def execute(words):
	try:
		...
		a = answer.decode()
		logup(update.message.chat_id, words, "DONE")
		return a
	except CalledProcessError as exc :
		e = exc.output.decode()
		logup(update.message.chat_id, words, "ERROR")
		return e
	else :
		assert 0
...
def start(bot, update):
	...
	bot.sendMessage(chat_id=update.message.chat_id, text="welcome,{update.message.from_user.username} just pressed start. This is indeed pretty useless")
...
def createap(bot, update):
	...
	bot.sendMessage(chat_id=update.message.chat_id, text="Ok, I see you're pimped for this command,{update.message.from_user.username}.\n i am too indeed...")
...
def help(bot, update):
	...
	bot.sendMessage(chat_id=update.message.chat_id, text="you can use the command /cmd [command] to execute something in the server. (Superuser/Admin)")
	bot.sendMessage(chat_id=update.message.chat_id, text="you can use the command /createap to create an AP. (Superuser/Admin)")
	bot.sendMessage(chat_id=update.message.chat_id, text="you can try the command /kill to close this bot. (Admin)")
	bot.sendMessage(chat_id=update.message.chat_id, text="you can try the command /logs to watch the log table. (Superuser/Admin)")
	bot.sendMessage(chat_id=update.message.chat_id, text="you can try the command /msg to send a message. (Admin)")
...
def cmd(bot, update):
	...
	if update.message.chat_id in sudoers:
		...
		if msg == "/cmd" :
			...
			words = msg.strip("/cmd")
			if words.find("reboot") > -1 or words.find("shut") > -1 or words.find("suspend") > -1 or words.find("hybernate") > -1 :
...
def msg(bot, update):
		...
		if update.message.chat_id in sudoers:
			if update.message.chat_id == GOD :
				bot.sendMessage(chat_id=GOD, text="Usage: /msg [username] text")
...
msg_handler = CommandHandler('msg', msg)				######TODO
